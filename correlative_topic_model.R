...
dloglike <- function(wsum, beta, mu, inv_Cov, z_dt, k){
  ...
  #微分関数の設定
  er <- beta - matrix(mu, nrow=d, ncol=k-1, byrow=T)
  dlogit <- (wsum - w*prob)[, -k]   #ロジットモデルの対数尤度の微分関数
  dmvn <- -t(inv_Cov %*% t(er))
...
#分散共分散行列のサンプリング結果
matplot(t(COV[1,  , ]), type="l", ylab="パラメータ")
matplot(t(COV[2,  , ]), type="l", ylab="パラメータ")
matplot(t(COV[3,  , ]), type="l", ylab="パラメータ")
matplot(t(COV[4,  , ]), type="l", ylab="パラメータ")
